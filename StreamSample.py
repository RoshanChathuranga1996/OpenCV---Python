...
def matchAd(fNo, adNo, stNo, di, sn):
    ...
    logger1.info('-------' * 5)
    logger3.info("<< Working with Ad {} ({}) >>".format(adNo, adName[adNo-1]))
    logger3.info('-------' * 5)
...
# Stream
path = "E:\\Opencv_project\\Videos\\Stream\\tv stream3.mp4"
...
print(" Total match frames count = {}".format(matchCount))
print("Total miss match frames count = {}".format(missMatchCount))
matchingPre = (matchCount / 179) * 100
